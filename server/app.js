...
function createApp() {
  ...
  app.get("/api/entries", async (_req, res) => {
    try {
      res.json((await getEntries()).map(withMeta));
    } catch (error) {
      sendError(res, error);
    }
  });
  ...
  app.post("/api/entries", async (req, res) => {
    try {
      res.status(201).json(withMeta(await createEntry(req.body)));
    } catch (error) {
      sendError(res, error.status ? error : Object.assign(error, { status: 400 }));
    }
  });
  ...
  app.put("/api/entries/:id", async (req, res) => {
    try {
      res.json(withMeta(await updateEntry(req.params.id, req.body)));
    } catch (error) {
      sendError(res, error.status ? error : Object.assign(error, { status: 400 }));
    }
  });
